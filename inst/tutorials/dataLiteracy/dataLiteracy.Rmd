...
```{r}
plot <- crime_data_stat %>% ggplot(aes(x=crime_yearmonth, y=count, fill=offense_category)) + geom_area(position="fill")
...
[The Bechdel Test](http://bechdeltest.com) is a test to determine whether women are represented fairly in movies by whether they are just talking about men, or talking about women. The following is a dataset where the proportions of each of these categories are tabulated for movies over a range of time.
