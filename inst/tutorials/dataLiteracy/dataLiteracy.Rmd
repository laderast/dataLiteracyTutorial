...
```{r echo=FALSE}
bmi_diabetes %>% tidyr::drop_na() %>% ggplot(aes(x=factor(1), fill=Diabetes)) + geom_bar(width=1, position = "stack")
```
...
Take a look at the barchart and pie chart. Which group (`male` or `female`) has the larger number of Diabetes patients?

```{r echo=FALSE}
outPlot <- bmi_diabetes %>% tidyr::drop_na() %>% ggplot(aes(x=factor(1), fill=Diabetes)) + geom_bar(width=1) + facet_grid(".~Gender")

outPlot + ggtitle("Stacked Bar Plot comparing Diabetes by Gender")

outPlot2 <- outPlot + coord_polar("y", start=0)
...
output$hist2_plot <- renderPlot({
  ...
  if(input$hist2_scale == "log"){
    outPlot <- outPlot + scale_x_log10(position="bottom")
  }

  outPlot <- outPlot + ggtitle(paste0("BMI scores (", input$hist2_scale, " scale)"))
...
```{r}
bmi_diabetes_small %>% ggplot(aes(x=Age, y=BMI)) + geom_point()
```
---
