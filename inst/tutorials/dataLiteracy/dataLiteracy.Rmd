...
We're going to look at data from a study called [NHANES (National Health and Nutrition Survey)](https://www.cdc.gov/nchs/nhanes/about_nhanes.htm). 
...
bins <- 30
  ...
  if(input$hist3_scale == "box+violin"){
    outPlot <- outPlot  +   
      ...
      geom_boxplot(aes(y=BMI), width=0.5) + 
      coord_flip()
  }
...
Time series data is everywhere. We are often interested in looking at overall trends, or differences in trends.
...
Here's what the data looks like as individual points.
...
Whickham %>% ggplot(aes(x=outcome, fill=smoker)) + geom_bar(color="black", position="fill")
```

```{r simpsons}
question("According to the data, if you are a smoker, are you more likely to live or not?",
...
The following is a plot of the outcome status versus the age of the patient in the dataset. 
...
Adjust the slider and see what happens to the number of deaths as we exclude the age of the highest participants.

```{r}
sliderInput("slidy", "Age Cutoff", min=min(Whickham$age)+1, max=max(Whickham$age)-1, value=c(84),step = 5)
plotOutput("distPlot")
```

```{r context="server"}
  output$distPlot <- renderPlot(
      exMelt() %>% ggplot(aes(x=outcome, fill=smoker)) + geom_bar(position="fill", color="black") #+
